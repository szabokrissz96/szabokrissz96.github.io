...
function configSigmaElements(config) {
    ...
    var a = [],
		...
		for (b in sigInst.clusters) 
            if (x == 1) {
  a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;background:' + b + ';display:inline-block"></div> Nem megafon '  + ' (' + sigInst.clusters[b].length + ' members)</a></div>'); (x++)
} 
    if (x == 2) {
  a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;background:' + b + ';display:inline-block"></div> Megafon '  + ' (' + sigInst.clusters[b].length + ' members)</a></div>');
}
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (2 >= a.length) this.results.html("<i>Legalább három karakterre keress.</i>");
        else {
